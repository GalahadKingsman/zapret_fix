@echo off
chcp 65001 > nul
:: Megafon Orel: Discord + YouTube + Telegram Desktop
:: Run setup-dns-megafon.bat and setup-*-hosts.bat first (once)
...
start "zapret: %~n0" /min "%BIN%winws.exe" --wf-tcp=80,443,5222,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=443,19294-19344,50000-50100,%GameFilterUDP% ^
...
--filter-udp=443 --hostlist="%LISTS%list-telegram.txt" --hostlist="%LISTS%list-general-user.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=11 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --new ^
...
--filter-udp=443 --ipset="%LISTS%ipset-telegram.txt" --dpi-desync=fake --dpi-desync-repeats=11 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --new ^
